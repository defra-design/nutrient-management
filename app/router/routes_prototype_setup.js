...
router.get(/end_to_end_setup_handler/, function (req, res) { 
    ...
    req.session.data.nextCropGroups.push(allFunctions.createCropGroup(1, 2025, [1,2], req.session.data.all_fields, 'grass', null, 'Crop group 1', '11', null, false))
    var applicationOne = allFunctions.addFertiliserApplication_v2 (req.session.data.all_fields, req.session.data.nextCropGroups, 1, '01/03/2024', 60, 30, 30, 0, 0, 1)
...
router.get(/onecrop_v5_setup_handler/, function (req, res) { 
    ...
    req.session.data.nextCropGroups.push(allFunctions.createCropGroup(1, 2025, [1,2,3,4,5], req.session.data.all_fields, 'Wheat-Winter', 'Skyfall', 'Crop group 1', '8', null, false))
    res.redirect('start')
})
...
router.get(/plan_setup/, function (req, res) { 
    ...
    req.session.data.oaktree_farm.storage_added = true
    req.session.data.nextCropGroups.push(allFunctions.createCropGroup(1, 2025, [8, 12], req.session.data.all_fields, 'Beans-Winter', 'Vespa', 'Crop group 1', null, null, true))
    req.session.data.nextCropGroups.push(allFunctions.createCropGroup(2, 2025, [1, 4, 5, 6, 7, 14], req.session.data.all_fields, 'Wheat-Winter', 'Skyfall', 'Crop group 2', '10 to 13', "12/03/2025", true))
    req.session.data.nextCropGroups.push(allFunctions.createCropGroup(3, 2025, [10, 11, 18, 19], req.session.data.all_fields, 'grass', null, 'Crop group 3', '12', null, false))
